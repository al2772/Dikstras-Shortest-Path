...
public class Graph<T> implements GraphADT<T> {
  ...
  protected class Path implements Comparable<Path> {
    ...
    public Path(Path copyPath, Edge extendBy) {
      // set the start and end vertex of the path
      this.start = copyPath.start;
      this.end = extendBy.target;

      // create a new object for the data sequence
      this.dataSequence = new LinkedList<>();
  ...
  protected Path dijkstrasShortestPath(T start, T end) {
    // check if there is in fact a valid path from start to end
    ...
    // create a priority queue to keep track of paths discovered
    PriorityQueue<Path> queue = new PriorityQueue<Path>(vertices.size());
    List<Vertex> visited = new LinkedList<Vertex>();

    // create base path with just the starting vertex
    Vertex first = vertices.get(start);
    Path shortestPath = new Path(first);

    // add this path to the priority queue
    queue.add(shortestPath);

    // go through the queue until it's empty
    while (!queue.isEmpty()) {

      // set shortest path to first in queue
      shortestPath = queue.peek();

      // check if the end was properly found
      if (shortestPath.end.data == end) {
        return shortestPath;
      }

      // add the paths extending off this one to the priority queue
      for (int i = 0; i < shortestPath.end.edgesLeaving.size(); i++) {

        Edge edge = shortestPath.end.edgesLeaving.get(i);

        if (!visited.contains(edge.target)) {
          queue.add(new Path(shortestPath, edge));
        }

      }

      // actually remove the top priority node
